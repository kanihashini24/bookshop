# -*- coding: utf-8 -*-
"""
Created on Fri Dec 23 11:00:59 2022
...
def database():
   name1=Name.get()
   gen=Gender.get()
   conn = sqlite3.connect('vishwa.db')
   with conn:
      cursor=conn.cursor()
   cursor.execute('CREATE TABLE IF NOT EXISTS userdata1 (Name TEXT, Gender TEXT)')
   cursor.execute('INSERT INTO userdata1 (Name, Gender) \
                  VALUES(?,?)', (name1, gen))
   conn.commit()
